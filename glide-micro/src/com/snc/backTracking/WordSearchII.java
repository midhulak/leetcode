...
public class WordSearchII {
    ...
    public List<String> findWords(char[][] board, String[] words) {
        ...
        for(int i = 0; i < board.length; i++){
            for(int j = 0; j < board[0].length; j++){
            	//check from board is that character present in trie and it is starting char of the word 
    ...
    private void search(char[][] board, int i, int j, TrieNode node, String word, Set<String> result){
        ...
        //if the next letter of teh word present we can add it to teh word string and keep on iterating.
